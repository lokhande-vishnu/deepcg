...
class CGD:
    ...
    def calculate_grad(self, grad_type, grads_and_vars):
        if grad_type == 1:  # sgd_update
            ...
            gvs = [(get_cgd(gv[0], gv[1], self.alpha, self.lamda,
                            grad_type), gv[1]) for gv in grads_and_vars]
        elif grad_type == 4:  # cgd_nn
            g0_cgd_nn = grads_and_vars[0][0]
            w0_cgd_nn = grads_and_vars[0][1]
            s0_cgd_nn, st, st_r, M = get_cgd(
                g0_cgd_nn, w0_cgd_nn, self.alpha, self.lamda, grad_type)
    ...
    def train(self, mnist, n_iters, print_iters, batch_size):
        ...
        if self.grad_type > 0:
            ...
            if self.grad_type == 4:
                ...
                update_for_non_j = [
                    (cal_grad_set(gv, self.alpha, self.lamda, 4), gv[1]) for gv in gv_non_j]
                ...
                update_for_j = [(get_cgd_with_st(
                    gv[0] / sum_fb_norm, gv[1], self.alpha, self.lamda), gv[1]) for gv in gv_j]
                # from IPython import embed; embed()
                gvs = update_for_non_j + update_for_j
            else:
                gvs = self.calculate_grad(self.grad_type, grads_and_vars)
            # end
            solver = self.opt.apply_gradients(
